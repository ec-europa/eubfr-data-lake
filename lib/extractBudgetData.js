...
const extractBudgetData = input => {
  ...
  const currenciesCombined = `${currenciesList}|${currenciesListSymbolic}`;
  const currenciesCombinedPattern = new RegExp(currenciesCombined, 'gi');
  const currenciesCombinedMatch = results.formatted.match(
    currenciesCombinedPattern
  );

...
describe('budgetFormatter module: extractMonetaryValue helper', () => {
  ...
  test("Can't handle: multilingual euro '2,5 millions d'euros'", () => {
    const result = extractMonetaryValue("2,5 millions d'euros");
    expect(result).toBe(0);
  });

  test("Can't handle: both multilingual countables and currencies 'Les Mureaux (alphabétisation + garderie): 7,4 millions de FF- Aulnay-sous-Bois (loges + gloriettes): 6,5 millions de FF'", () => {
    const result = extractMonetaryValue(
      'Les Mureaux (alphabétisation + garderie): 7,4 millions de FF- Aulnay-sous-Bois (loges + gloriettes): 6,5 millions de FF'
    );
    expect(result).toBe(0);
  });
  ...
  test("Can handle: 'FEDER (Objectif 1, 1994-1999): 3 103 391 euros (20 356 911 FRF)'", () => {
    const result = extractMonetaryValue(
  ...
  test("Can handle: 'Feder (Objectif 5b) 80 798 euros (530 000 FRF)'", () => {
    const result = extractMonetaryValue(
  ...
  test("Can handle: dot notation and plural euro '43.000.000 euros'", () => {
    const result = extractMonetaryValue('43.000.000 euros');
    expect(result).toBe('43,000,000');
  });

  test("Can handle: dot notation and singular million 'EUR 3.196 million - Cohesion Fund'", () => {
    const result = extractMonetaryValue('EUR 3.196 million - Cohesion Fund');
    // Later { _input: '3,196 m', _value: 3196000000 } in numeral!
    expect(result).toBe('3,196 m');
  });

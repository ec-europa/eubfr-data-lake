...
describe('extractBudgetData helper', () => {
  ...
  test('Should take into account millions in supported currencies', () => {
    const input = "1,3 million d' Ecus dont environ 50% UE  et Région wallonne";
    const result = extractBudgetData(input);
    expect(result.value).toBe('1.3 m');
    expect(result.currency).toBe('ECU');
  });

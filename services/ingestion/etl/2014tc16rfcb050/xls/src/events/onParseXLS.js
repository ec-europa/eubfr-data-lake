  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      readStream.on('end', async () => {
        ...
        const recordsAgg = [];
        // stores formatted version of recordsAgg, i.e. trimmed object keys.
        const recordsImproved = [];
        ...
        // Improve records after the aggregation.
        recordsAgg.forEach(record => {
          const recordImproved = {};

          Object.keys(record).forEach(field => {
            const newField = field.trim();
            recordImproved[newField] = record[field];
          });

          recordsImproved.push(recordImproved);
        });

        recordsImproved.forEach(record => {

...
export const handler = async (event, context) => {
  ...
  try {
    ...
    if (!ensureExtensions({ file: key, extensions: ['.ods'] })) {
      throw new Error('ODS file expected for this ETL.');
    }
    ...
    await messenger.send({
      message: {
        computed_key: key,
        status_message: 'Start parsing ODS...',
    ...
    return new Promise((resolve, reject) => {
      ...
      readStream.on('end', async () => {
        ...
        const improvedData = parsedRows.map(row => {
          const improvedRow = {};
        ...
        await messenger.send({
          message: {
            computed_key: key,
            status_message:
              'ODS parsed successfully. Results will be uploaded to ElasticSearch soon...',
            status_code: STATUS.PARSED,
          },
          to: ['logs'],
        });

        return resolve('ODS parsed successfully');
      });
    });
  } catch (e) {
    throw e;
